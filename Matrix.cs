...
namespace HT.Algorithm
{
	...
	*/
	public class Matrix 
	{
        ...
        private decimal eps = decimal.Parse("0.0");			// 缺省精度
        private decimal[] elements = null;	// 矩阵数据缓冲区
		...
		public Matrix(decimal[,] value)
		{
            ...
            decimal[] data = new decimal[numRows * numColumns];
			int k = 0;
		 ...
		 */
		public Matrix(int nRows, int nCols, decimal[] value)
		{
			numRows = nRows;
			numColumns = nCols;
			Init(numRows, numColumns);
		 ...
		 */
		public Matrix(int nSize, decimal[] value)
		{
			numRows = nSize;
			numColumns = nSize;
			Init(nSize, nSize);
		 ...
		 * @param newEps - 新的精度值
		 */
		public void SetEps(decimal newEps)
		{
			eps = newEps;
		}
		 ...
		 * 取矩阵的精度值
		 * 
		 * @return decimal型，矩阵的精度值
		 */
		public decimal GetEps()
		{
			return eps;
		}
		 ...
		 * @return decimal[]对象
		 */
		public static implicit operator decimal[](Matrix m)
		{
			return m.elements;
		}
		 ...
		 */
        public bool SetElement(int nRow, int nCol, decimal value)
		{
		 ...
		 * @param nRow - 元素的行
		 * @param nCol - 元素的列
		 * @return decimal 型，指定元素的值
		 */
        public decimal GetElement(int nRow, int nCol) 
		{
		 ...
		 * 获取矩阵的数据
		 * 
		 * @return decimal型数组，指向矩阵各元素的数据缓冲区
		 */
		 ...
		 */
        public int GetRowVector(int nRow, decimal[] pVector) 
		{
			for (int j=0; j<numColumns; ++j)
		 ...
		 */
        public int GetColVector(int nCol, decimal[] pVector) 
		{
			for (int i=0; i<numRows; ++i)
		...
		public override int GetHashCode()
		{
			...
			return (int)Math.Sqrt(double.Parse(sum.ToString()));
		}
		 ...
		 */
        public Matrix Multiply(decimal value) 
		{
			// 构造目标矩阵
		...
		public bool Multiply(Matrix AR,  Matrix AI,  Matrix BR,  Matrix BI, Matrix CR, Matrix CI) 
		{
			...
			for (int i=0; i<AR.GetNumRows(); ++i)
			{
				for (int j=0; j<BR.GetNumColumns(); ++j)
				{
					...
					{
                        decimal p = AR.GetElement(i, k) * BR.GetElement(k, j);
                        decimal q = AI.GetElement(i, k) * BI.GetElement(k, j);
                        decimal s = (AR.GetElement(i, k) + AI.GetElement(i, k)) * (BR.GetElement(k, j) + BI.GetElement(k, j));
						vr += p - q;
						vi += s - p - q;
					}
		...
		public bool InvertGaussJordan()
		{
			...
			{ 
				...
				l=k*numColumns+k;
				elements[l]=decimal.Parse("1.0")/elements[l];
				for (j=0; j<=numColumns-1; j++)
				{
					if (j != k)
					{ 
		...
		{ 
			...
			// 临时内存
            decimal[] pTmp = new decimal[numColumns];
			...
			{ 
				...
				elements[numColumns*numColumns-1]=1/w;
				for (i=1; i<=numColumns-1; i++)
		...
		{ 
			...
			// 上三角元素
            decimal[] t = new decimal[numColumns];
			// 下三角元素
            decimal[] tt = new decimal[numColumns];
			...
			// 临时缓冲区
            decimal[] c = new decimal[numColumns];
            decimal[] r = new decimal[numColumns];
            decimal[] p = new decimal[numColumns];
		 ...
		 * 求行列式值的全选主元高斯消去法
		 * 
		 * @return decimal型，行列式的值
		 */
		public decimal ComputeDetGauss()
		{ 
		 ...
		 * @return bool型，求解是否成功
		 */
		public bool ComputeDetCholesky(ref decimal realDetValue)
		{ 
			int i,j,k,u,l;
			decimal d;
			...
			elements[0]=decimal.Parse(Math.Sqrt(double.Parse(elements[0].ToString())).ToString());
			d=elements[0];
			...
			{ 
				...
				elements[l]= decimal.Parse(Math.Sqrt(double.Parse(elements[l].ToString())).ToString());
				d=d*elements[l];
		...
		{ 
			...
			{ 
				...
				alpha=u*decimal.Parse(Math.Sqrt(double.Parse(alpha.ToString())).ToString());
				if (alpha == 0)
					return false;

				u=decimal.Parse(Math.Sqrt(double.Parse((2 * alpha * (alpha-elements[l])).ToString())).ToString());
				if ((u+1)!=1)
				{ 
					...
					{ 
						...
						{ 
							p=i*numRows+j; 
							mtxQ.elements[p]=mtxQ.elements[p] - 2 * t * elements[i*numColumns+k];
						}
					}
					...
					{ 
						...
						{ 
							p=i*numColumns+j; 
							elements[p]=elements[p]- 2 * t * elements[i*numColumns+k];
						}
					}
	            
					elements[l]=alpha;
					for (i=k+1; i<=numRows-1; i++)
						elements[i*numColumns+k]=0;
				}
			}
		 ...
		 * @return bool型，求解是否成功
		 */
		public bool SplitUV(Matrix mtxU, Matrix mtxV, decimal eps)
		{ 
			int i,j,k,l,it,ll,kk,ix,iy,mm,nn,iz,m1,ks;
			decimal d,dd,t,sm,sm1,em1,sk,ek,b,c,shh;
			decimal[] fg = new decimal[2];
			decimal[] cs = new decimal[2];
			...
			decimal[] s = new decimal[ka];
			decimal[] e = new decimal[ka];
			decimal[] w = new decimal[ka];
			...
			{ 
				...
				{ 
					...
					{ 
						...
						s[kk-1]=decimal.Parse(Math.Sqrt(double.Parse(d.ToString())).ToString());
						if (s[kk-1]!=0) 
						{ 
							ix=(kk-1)*n+kk-1;
							if (elements[ix]!=0)
					...
					{ 
						...
						e[kk-1]=decimal.Parse(Math.Sqrt(double.Parse(d.ToString())).ToString());
						if (e[kk-1]!=0)
						{ 
							if (e[kk]!=0)
							{ 
			...
			{ 
				...
				{ 
					for (i=1; i<=m; i++)
						mtxU.elements[(i-1)*m+j-1]=0;
					mtxU.elements[(j-1)*m+j-1]=1;
				}
			}
			...
			{ 
				...
				{ 
					...
					{ 
						...
						mtxU.elements[iz]=1+mtxU.elements[iz];
						if (kk-1>=1)
						{
							for (i=1; i<=kk-1; i++)
								mtxU.elements[(i-1)*m+kk-1]=0;
						}
					}
					else
					{ 
			...
			{ 
				...
				for (i=1; i<=n; i++)
					mtxV.elements[(i-1)*n+kk-1]=0;
	        
				mtxV.elements[iz-n]=1;
			}
	    
			for (i=1; i<=m; i++)
				for (j=1; j<=n; j++)
					elements[(i-1)*n+j-1]=0;
			...
			{ 
				...
				{ 
					...
					{ 
						...
						if ((b!=0)||(c!=0))
						{ 
							shh=decimal.Parse(Math.Sqrt(double.Parse((b*b+c).ToString())).ToString());
							if (b<0) 
								shh=-shh;
		 ...
		 * 内部函数，由SplitUV函数调用
		 */
		private void ppp(decimal[] a, decimal[] e, decimal[] s, decimal[] v, int m, int n)
		{ 
			int i,j,p,q;
			decimal d;
		 ...
		 * 内部函数，由SplitUV函数调用
		 */
		private void sss(decimal[] fg, decimal[] cs)
		{ 
			decimal r,d;
	    
			if ((Math.Abs(fg[0])+Math.Abs(fg[1]))==0)
			{ 
				cs[0]=1; 
				cs[1]=0; 
				d=0;
			...
			{ 
				d=decimal.Parse(Math.Sqrt(double.Parse((fg[0]*fg[0]+fg[1]*fg[1]).ToString())).ToString());
		 ...
		 * @return bool型，求解是否成功
		 */
		public bool InvertUV(Matrix mtxAP, Matrix mtxU, Matrix mtxV, decimal eps)
		{ 
			int i,j,k,l,t,p,q,f;
		 ...
		 * @return bool型，求解是否成功
		 */
		public bool MakeSymTri(Matrix mtxQ, Matrix mtxT, decimal[] dblB, decimal[] dblC)
		{ 
			int i,j,k,u;
			decimal h,f,g,h2;
			...
			{ 
				...
				{ 
					dblC[i]= decimal.Parse(Math.Sqrt(double.Parse(h.ToString())).ToString());
					u=i*numColumns+i-1;
					if (mtxQ.elements[u] > 0) 
						dblC[i]=-dblC[i];
			...
			{ 
				...
				u=i*numColumns+i;
				dblB[i]=mtxQ.elements[u]; mtxQ.elements[u]=1;
				if (i-1>=0)
				{
					for (j=0; j<=i-1; j++)
					{ 
						mtxQ.elements[i*numColumns+j]=0; 
						mtxQ.elements[j*numColumns+i]=0;
					}
				}
			}
		 ...
		 * @return bool型，求解是否成功
		 */
		public bool ComputeEvSymTri(decimal[] dblB, decimal[] dblC, Matrix mtxQ, int nMaxIt, decimal eps)
		{
			int i,j,k,m,it,u,v;
			decimal d,f,h,g,p,r,e,s;
			...
			{ 
				...
				{ 
					...
					{ 
						...
						r= decimal.Parse(Math.Sqrt(double.Parse((p*p+1).ToString())).ToString());
						if (p>=0) 
							dblB[j]=dblC[j]/(p+r);
						else 
							dblB[j]=dblC[j]/(p-r);
						...
						{ 
							...
							{ 
								e=dblC[i]/p; 
								r=decimal.Parse(Math.Sqrt(double.Parse((e*e+1).ToString())).ToString());
								dblC[i+1]=s*p*r; 
								s=e/r; 
								e=1 / r;
							}
		 ...
		 * @return bool型，求解是否成功
		 */
		public bool ComputeEvHBerg(decimal[] dblU, decimal[] dblV, int nMaxIt, decimal eps)
		{ 
			int m,it,i,j,k,l,ii,jj,kk,ll;
			decimal b,c,w,g,xy,p,q,r,x,s,e,f,z,y;
			...
			{ 
				...
				{ 
					...
					w=b*b-4*c;
					y=decimal.Parse(Math.Sqrt(double.Parse(Math.Abs(w).ToString())).ToString());
					if (w>0)
					{ 
						xy=1;
						if (b<0) 
				...
				{ 
					...
					{ 
						...
						if ((Math.Abs(p)+Math.Abs(q)+Math.Abs(r))!=0)
						{ 
							xy=1;
							if (p<0) 
								xy=-1;
							s=xy*decimal.Parse(Math.Sqrt(double.Parse((p*p+q*q+r*r).ToString())).ToString());
							if (k!=l) 
								elements[k*n+k-1]=-s;
							e=-q/s; 
							f=-r/s; 
		 ...
		 * @return bool型，求解是否成功
		 */
		public bool ComputeEvJacobi(decimal[] dblEigenValue, Matrix mtxEigenVector, int nMaxIt, decimal eps)
		{ 
			int i,j,p = 0,q = 0,u,w,t,s,l;
			decimal fm,cn,sn,omega,x,y,d;
			...
			for (i=0; i<=numColumns-1; i++)
			{ 
				mtxEigenVector.elements[i*numColumns+i]=1;
				for (j=0; j<=numColumns-1; j++)
					if (i!=j) 
						mtxEigenVector.elements[i*numColumns+j]=0;
			}
			...
			{ 
				...
				s=q*numColumns+q;
				x=-elements[u]; 
				y=(elements[s]-elements[w])/2;
				omega=x / decimal.Parse(Math.Sqrt(double.Parse((x*x+y*y).ToString())).ToString());

				if (y < 0) 
					omega=-omega;

				sn=1 + decimal.Parse(Math.Sqrt(double.Parse((1-omega*omega).ToString())).ToString());
				sn=omega/ decimal.Parse(Math.Sqrt(double.Parse( (2*sn).ToString())).ToString());
				cn=decimal.Parse(Math.Sqrt(double.Parse(( 1 - sn*sn).ToString())).ToString());
				fm=elements[w];
		 ...
		 * @return bool型，求解是否成功
		 */
		public bool ComputeEvJacobi(decimal[] dblEigenValue, Matrix mtxEigenVector, decimal eps)
		{ 
			int i,j,p,q,u,w,t,s;
			decimal ff,fm,cn,sn,omega,x,y,d;
			...
			ff=decimal.Parse(Math.Sqrt(double.Parse((2 * ff).ToString())).ToString());
			ff=ff/(1 * numColumns);
